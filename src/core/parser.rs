...
"#;
    ...
    #[test]
    fn test_extract_debug_delete_rs_format() {
        // Exact format from problematic split output
        let response = concat!(
        ...
        assert_eq!(files[0].path, Some(PathBuf::from("src/user_service/delete.rs")));
        assert!(files[0].content.contains("delete_user"));
        assert!(!files[0].content.contains("```"), "Content should not contain backticks");
    }

    #[test]
    fn test_nested_fences_in_worksplit() {
        // LLM wraps with ~~~worksplit but also includes path-as-heading inside
        let response = concat!(
            "~~~worksplit\n",
            "src/user_service/delete.rs\n",
            "```rust\n",
        ...
        let files = extract_code_files(response);
        assert_eq!(files.len(), 1);
        // Should strip both the path heading and backticks
        assert!(files[0].content.contains("pub fn delete()"));
        assert!(!files[0].content.contains("```"), "Should strip backticks");
        assert!(!files[0].content.contains("src/user_service"), "Should strip path heading");
    }

    #[test]
    fn test_nested_backticks_in_worksplit() {
        // LLM wraps with ~~~worksplit and also includes backticks inside
        let response = concat!(
            "~~~worksplit\n",
            "```rust\n",
            "fn main() {}\n",
